...
<head>
    ...
    <script type="text/javascript">
        ...
        var MatTemplate_initNextLevel = "<?=site_url('MatTemplate/initNextLevel')?>";
        var MatTemplate_getBOMCat_ID1 = "<?=site_url('MatTemplate/getBOMCat_ID1')?>";
        var MatTemplate_getBOMCat_ID2 = "<?=site_url('MatTemplate/getBOMCat_ID2')?>";
...
<!--                <li class="row-item">
    ...
    --> 
 <li class="row-item">
                    <div class="label-wrap"> <label  class="tit" for="filter-user">自定义类：</label></div>
                    <div class="ctn-wrap"><span class="ui-combo-wrap" id="goodsTempl">
                    <input type="text" name="filter-user" id="filter-user" onblur="getName()" class="input-txt" style="width:172px" autocomplete="off" />
                    <span class="trigger"></span>
                    </span></div>
                </li>
                <li class="row-item">
                    <div class="label-wrap"> <label  class="tit" for="filter-user"></label></div>
           ...
           <li class="row-item">
                ...
                <li class="row-item">
                    <div class="label-wrap"><label for="selfClass">单位类别</label></div>
                    <div class="ctn-wrap">
                        <select name="IsKey" id="unitClass"  style="height: 30px;width:100px">
...
<script>
    function init() {
        $.ajax({
            type: "get",
            async:false,
            url: MatTemplate_initUnitClass,
            dataType: 'json',
            success: function (result) {
                $("#unitClass").text("");
                for (let i = 0; i < result.name.length; i++) {
                    $("#unitClass").append("<option value=" + result.id[i] + ">" + result.name[i] + "</option>");
                }
            },
            error: function () {
                alert('Error loading HTML document');

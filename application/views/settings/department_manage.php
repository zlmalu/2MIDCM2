...
var basedata_upDepartment = "<?=site_url('basedata/upDepartment')?>";
var department_initDepartment = "<?=site_url('department/initDepartment')?>";
...
/*        $(function() {
            $.ajax({
                type: "get",
                async: false,
                url: department_initDepartment,
                dataType: 'json',
                success: function (result) {
                    $("#upDepartment").text("");
                    $("#upDepartment").append("<option value=0>无</option>");
                    for (let i = 0; i < result.name.length; i++) {
                        $("#upDepartment").append("<option value=" + result.id[i] + ">" + result.name[i] + "</option>");
                    }
                },
                error: function () {
...
<div id="manage-wrap" class="manage-wrap">
	<form id="manage-form" action="">
		<ul class="mod-form-rows">
            <li class="row-item">
                <div class="label-wrap"> <label  class="tit" for="filter-user">所属部门：</label></div>
                <div class="ctn-wrap"><span class="ui-combo-wrap" id="upDepartment">
                    <input type="text" name="filter-user" id="filter-upName" class="input-txt" autocomplete="off" />

...
class WPCat extends CI_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->purview_model->checkpurview(152);
...
//        $data=array("name"=>$name,"Cat_ID2"=>$Cat_ID2);
    ...
    public function save()
    {
        ...
        if ($act=='add') {
            $this->purview_model->checkpurview(153);
            $name = $data['Name'];
            $id = $this->mysql_model->db_inst(WPCAT, $data);
            $this->cache_model->delsome(BETWEENUNIT);
            $this->data_model->logs('新增工作流程类别信息id='.$id.' 名称:'.$name);
            //回传数据
...
//                'Status' => $data['Status'],'BU_Cat' => $data['BU_Cat']);
            ...
            $this->purview_model->checkpurview(154);
            $name = $data['Name'];
            $Desc = $data['Desc'];
...
//                    'Status' => $data['Status'],'BU_Cat' => $data['BU_Cat']);
    ...
    public function del(){
        $this->purview_model->checkpurview(155);
...
//                $name = join(',',$name);
//            }
        $sql = $this->mysql_model->db_del(WPCAT,'(PK_WPCat_ID in('.$id.'))');
        if ($sql) {
            $this->cache_model->delsome(WPCAT);

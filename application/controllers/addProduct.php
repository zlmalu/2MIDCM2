...
class AddProduct extends CI_Controller
{
    ...
    {
        $sql = "select Name from t_bom_category2 where level=1";
        $result = $this->db->query($sql);
        $level1= array();
        foreach ($result->result() as $row)
        {
            $arr = object_array($row);
            array_push($level1,$arr["Name"]);
        }
        $sql1 = "select Name from t_bom_category2 where level=2";
        $result1 = $this->db->query($sql1);
        $level2= array();
        foreach ($result1->result() as $row)
        {
            $arr = object_array($row);
            array_push($level2,$arr["Name"]);
        }
        $data=array("level1"=>$level1,"level2"=>$level2);

function initGrid() {
	var e = Public.setGrid(),
		t = ["操作","模板编号","大类","小类","大类ID","小类ID", "名称", "描述", "单位类别", "属性"],
...
function initEvent() {
	...
	$("#search").on("click", function(e) {
		e.preventDefault();
		var t = "输入模板编号/ 大类 / 小类 / 名称查询" === $_matchCon.val() ? "" : $.trim($_matchCon.val());
...
var searchFlag = !1,
	filterClassCombo, handle = {
		operate: function(e, t) {
			...
			$.dialog({
				title: i,
				content: "url:"+settings_bom_templ_manage,
				data: a,
				width: 640,
				height: 422,
				max: !1,
		...
		del: function(e) {
			$.dialog.confirm("删除的单位将不能恢复，请确认是否删除？", function() {
				...
				Public.ajaxPost(MatTemplate_del, {
					id: e
				}, function(t) {
					if (t && 200 == t.status) {
						var i = t.data.id || [];
